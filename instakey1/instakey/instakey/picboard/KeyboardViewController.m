...
@property (nonatomic, strong) NSArray *symbolsRowOne;
@property (nonatomic, strong) NSArray *symbolsRowTwo;
...
- (void)viewDidLoad {
    ...
    if (![self isOpenAccessGranted]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            _textLabelFullAccess = [[UILabel alloc] initWithFrame:CGRectMake(width/2-100, 85, 200, 40)];
            _textLabelFullAccess.text = @"Please enable full access.";
            _textLabelFullAccess2 = [[UILabel alloc] initWithFrame:CGRectMake(width/2-112, 105, 250, 40)];
    ...
    NSString *sessionKey = [[[NSUserDefaults alloc] initWithSuiteName:@"group.com.jackrogers.instakey"] objectForKey:@"accessToken"];
    if (!sessionKey) {
        _loggedInInsta = NO;
        dispatch_async(dispatch_get_main_queue(), ^{
            _textLabelFullAccess = [[UILabel alloc] initWithFrame:CGRectMake(width/2-100, 85, 250, 40)];
    ...
    reach.unreachableBlock = ^(Reachability*reach) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            _textLabelFullAccess = [[UILabel alloc] initWithFrame:CGRectMake(12, 85, width-12, 40)];
...
- (void)loadDecimalKeyboard {
    ...
    NSArray *rowOne = @[@"onebutton.png", @"twobutton.png", @"threebutton.png", @"fourbutton.png", @"fivebutton.png", @"sixbutton.png", @"sevenbutton.png", @"eightbutton.png"];
    _symbolsRowOne = @[@"1", @"2", @"3", @"4", @"5", @"6", @"7", @"8"];
    NSArray *rowTwo = @[@"ninebutton.png", @"zerobutton.png", @"^button.png", @"asteriskbutton.png", @"+button.png", @"dashbutton.png", @"slashbutton.png", @"=button.png"];
    _symbolsRowTwo = @[@"9", @"0", @"^", @"*", @"+", @"-", @"/", @"="];
    int counter = 0;
    ...
    NSLog(@"dist: %d", dist);
    for (NSString *name in rowOne) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.view addSubview:button];
        button.frame = CGRectMake(0, 0, 28.0, 25.0);
        [button setBackgroundImage:[UIImage imageNamed:name] forState:UIControlStateNormal];
        [button setTranslatesAutoresizingMaskIntoConstraints:NO];
        button.tag = counter;
        [button addTarget:self action:@selector(insertRowOneSymbol:) forControlEvents:UIControlEventTouchUpInside];
        [_keyboardButtons addObject:button];
        [self.view bringSubviewToFront:button];
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.view.mas_top).with.offset(5.0); //with is an optional semantic filler
            make.left.equalTo(self.view.mas_left).with.offset(6 + (dist+28)*counter);
        }];
    ...
    counter = 0;
    for (NSString *name in rowTwo) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(0, 46, 28.0, 25.0);
        [button setBackgroundImage:[UIImage imageNamed:name] forState:UIControlStateNormal];
        [button setTranslatesAutoresizingMaskIntoConstraints:NO];
        [self.view addSubview:button];
        button.tag = counter;
        [button addTarget:self action:@selector(insertRowTwoSymbol:) forControlEvents:UIControlEventTouchUpInside];
        [_keyboardButtons addObject:button];
        [self.view bringSubviewToFront:button];
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.view.mas_top).with.offset(46.0); //with is an optional semantic filler
            make.left.equalTo(self.view.mas_left).with.offset(6 + (dist+28)*counter);
        }];
        counter++;
    }
...
- (void)insertRowOneSymbol:(id)sender {
    UIButton *button = (UIButton *)sender;
    int tag = (int)button.tag;
    [self.textDocumentProxy insertText:[_symbolsRowOne objectAtIndex:tag]];
}

- (void)insertRowTwoSymbol:(id)sender {
    UIButton *button = (UIButton *)sender;
    int tag = (int)button.tag;
    [self.textDocumentProxy insertText:[_symbolsRowTwo objectAtIndex:tag]];
}
...
- (void)addPhotoButton:(NSString*)nameOfImage {
    ...
    UILabel *textView = [[UILabel alloc] initWithFrame:CGRectMake((150 * _numberOfImagesAlreadyLoaded) + 10, 140, 120, 10)];
    textView.text = [_instaNames objectAtIndex:_numberOfImagesAlreadyLoaded];
    textView.textColor = [UIColor colorWithRed:1/255.0f green:246/255.0f blue:41/255.0f alpha:1.0f];
    textView.backgroundColor = [UIColor clearColor];
    textView.font = [UIFont systemFontOfSize:14];
    
    UILabel *description = [[UILabel alloc] initWithFrame:CGRectMake(150 * _numberOfImagesAlreadyLoaded + 5, 160, 120, 40)];
...
- (void)imageButtonPressed:(id)sender {
    ...
    [self.textDocumentProxy insertText:[NSString stringWithFormat:@"%@ ", [_instaLinks objectAtIndex:tag]]];
}
